...
import { Button, Card, message, Space, Input, Select } from 'antd';
...
const StaticBeta: React.FC = () => {
    //
    const [data, setData] = useState<ChartOption>({title: '######', categoryData: [], candlestickData: [[]], volumeData: [[]], markPointData: []});

    const [exchange, setExchange] = useState<string>("SH")

    const handleExchange = (value: any) => {
        setExchange(value);
    }

    const selectExchange = (
        <Select defaultValue="SH" className="select-before" onChange={handleExchange}>
    ...
    return (
        <PageContainer>
            <Card>
                <Space>
                    <Search placeholder="input stock code" addonBefore={selectExchange} enterButton="Start" defaultValue="000000"
            ...
            <Card>
                <ReactEcharts option={getOption(data)} style={{width: 400, height:400}}/>
            </Card>
        </PageContainer>
    );
};

export default StaticBeta;