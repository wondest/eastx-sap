...
class ApiHttpOperator(BaseOperator):
    """
    ...
    def check_response(response):
        ...
        if result is None:
            raise AirflowException("response is none")

        if not result.get('success'):
            raise AirflowException("response failed,errorCode=%s,errorMessage=%s" % (result.get('errorCode'), result.get('errorMessage')))
